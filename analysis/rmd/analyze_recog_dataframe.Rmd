...
## Analysis of accuracies across all three groups: communication, yoked, shuffled
```{r}
...
d.acc %>%
  group_by(version) %>%
  summarize(group_accuracy=mean(accuracy))
```
...
d.acc <- read_csv('../../results/graphical_conventions_recog_data.csv') %>%
  ...
  group_by(version,orig_gameID,repetition) %>%
  summarize(accuracy=mean(correct))
```

```{r}
m1a <- summary(lmer(accuracy ~ version * repetition + (1 + version * repetition | orig_gameID),
              data = d.acc))
summary(m1a)
...
m1b <- summary(lmer(accuracy ~ version * repetition + (1 + repetition | gameID),
              data = d.acc))
summary(m1b)
