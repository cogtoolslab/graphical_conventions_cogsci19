...
lmer.within <- M_mat %>%
    ...
    ungroup()
summary(lmer(sim ~ phase + (1 | gameID) + (1 | target), data = lmer.within))
```
...
summary(lmer(sim ~ poly(repetition, 2) + (1 + repetition | gamepair) , data = gamepair.df))
```

```{r}
ggplot(gamepair.df, aes(x = as.character(repetition), y = sim)) + 
  #geom_line(alpha = 0.1) +
  geom_boxplot() +
  theme_few() +
  facet_wrap(~ target) 
...
```{r}
empirical.across <- compute_across_similarity(M_mat, 'empirical',nboot = 500)
