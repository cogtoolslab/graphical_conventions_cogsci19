...
permuted_tstats.within <- map_dbl(seq_len(1000), ~ M_mat %>%
 # scrambles repetition
  ...
  map(~ tidy(., effects = 'fixed')[[2]][[2]]) %>%
  unlist())

cat('CI for within analysis=', sort(permuted_tstats.within)[25], sort(permuted_tstats.within)[975])
summary(lmer(sim ~ poly(rep1,2) + (1 + rep1 | gameID) + (1 + rep1 | target), data = lmer.within))
```
...
true_lmer <- lmer(sim ~ poly(repetition, 2) + (1 + repetition | gamepair) + (1 + repetition | target), data = gamepair.df)
true_tstat <- coef(summary(true_lmer))[,"t value"][2]

permuted_tstats <- map_dbl(seq_len(1000), ~ M_mat %>%
 # scrambles repetition
...
cat('CI=', sort(permuted_tstats)[25], sort(permuted_tstats)[975])

ggplot(data.frame(permuted = permuted_tstats, true = true_tstat),
       aes(x = 0, y = permuted)) +
