...
```{r}
crazies.run3 <- unique((d.run3.raw %>% filter(condition == 'repeated' & numStrokes > 25) %>% select(gameID))$gameID)
#crazies.pilot1 = c(crazies.pilot1, 
#            '0766-fcb90e7e-bf4a-4a46-b6d6-3165b6c12b88', 
#            '7024-8ac78089-539a-428b-9d0e-b52c71a0a1b4')
d.run3 <- d.run3.raw %>% filter(!(gameID %in% crazies.run3))
...
```{r}
summary(lmer(numStrokes ~ repetition + (1 + repetition| gameID) + (1 + repetition| target), data = d.run3 %>%   
  filter(condition == 'repeated') %>%
...
```{r}
crazies.run4 <- unique((d.run4.raw %>% filter(condition == 'repeated' & numStrokes > 25) %>% select(gameID))$gameID)
