...
def preprocess_features_for_pca(Features, Y):
    '''
    ...
    _Y = Y.sort_values(['run_num','gameID','repetition','target'])
    inds = np.array(_Y.index)
    _Features = Features[inds] scoring
    _Y = _Y.reset_index(drop=True) # reset pandas dataframe index
    return _Features, _Y    
...
def apply_pca_and_save(F, layer_num, data_type, num_pcs = 512, out_dir='/data/jefan/graphical_conventions/features'):    
    ...
    # b/c you have not applied channel-wise normalization yet, is do it here, AFTER applying PCA
    F_ = normalize(F_)
    ...
    print('Saved PC-transformed features out to {}! Note: These are generated by first applying PCA *then* doing channel-wise normalization.').format(out_dir)
    return F_  
...
if __name__ == "__main__":
    ...
    _FeaturesPCA, _YPCA = preprocess_features_for_pca(Features, Y)

    if args.test==False:
        layer = save_features(_Features, _Y, args.layer_ind, args.data_type,out_dir = args.out_dir)   
        _Features_PCA = apply_pca_and_save(_FeaturesPCA, args.layer_ind, args.data_type, num_pcs = args.num_pcs, out_dir=args.out_dir)
